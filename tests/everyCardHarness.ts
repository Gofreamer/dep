/**
 * EVERY-CARD PLAYABLE — harness compartilhado (executa CardDefs de verdade).
 *
 * A suíte completa vive aqui para ser reutilizada por:
 *   - tests/every-card-playable.test.ts  (pack JET + fixture NEXO)
 *   - tests/every-card-jet.test.ts       (SOMENTE o JET production pack)
 *
 * Para cada carta o harness valida função real, joga a carta, resolve
 * targets/choices/triggers, verifica invariantes (conservação, UIDs,
 * diagnóstico) e garante que ambos os lados seguem agindo.
 */
...
function buildDeck(card: CardDef): CardDef[] {
  ...
  // cartas de equipe com restriction factionInPlay precisam de um agente da
  //quela faccao no deck para o harness satisfazer a restriction de verdade.
  if (card.kind === 'ACTION') {
    for (const rest of (card as ActionDef).restrictions ?? []) {
      if (rest.type === 'factionInPlay' && rest.faction) {
        const fb = registry.allCards().find((d) => d.kind === 'CHARACTER' && (d as CharacterDef).faction === rest.faction && (d as CharacterDef).stage === 0);
        if (fb && !deck.includes(fb)) deck.push(fb);
      }
    }
  }
...
export function runEveryCardSuite(cards: CardDef[], titles?: { gameplay?: string; playable?: string; integrity?: string }): void {
  const ALL = cards;
  describe(titles?.gameplay ?? 'TODA carta tem função real de gameplay', () => {
  ...
  describe(titles?.playable ?? 'TODA carta é jogável de verdade (execução real)', () => {
    for (const def of ALL) {
      it(`${def.id} — ${def.name}`, () => {
  ...
  describe(titles?.integrity ?? 'CardDefs — integridade estrutural', () => {
